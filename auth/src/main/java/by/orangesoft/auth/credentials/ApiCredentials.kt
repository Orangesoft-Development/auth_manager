...
sealed class ApiCredentials(@AuthMethod val method: String) {
    ...
    class Phone(val firebase_id: String?, val phone: String, val sms_code: String, @AuthMethod method: String = AuthMethod.PHONE) : ApiCredentials(method) {
        companion object {
            fun fromPhoneCredential(id: String?, authMethod: PhoneAuthMethod): Phone =
                Phone(
                    id,
                    authMethod.phone,
                    authMethod.code
    ...
    sealed class Social(
        ...
        class Google(accessToken: String) : Social(
            AuthMethod.GOOGLE, accessToken)
        class Facebook(accessToken: String) : Social(
            AuthMethod.FACEBOOK, accessToken)
        class Apple(accessToken: String) : Social(
            AuthMethod.APPLE, accessToken)
    }
    ...
    sealed class ApiFirebase(
        ...
        class Google(accessToken: String) : ApiFirebase(
            FirebaseMethod.GOOGLE, accessToken)
        class Facebook(accessToken: String) : ApiFirebase(
            FirebaseMethod.FACEBOOK, accessToken)
        class Apple(accessToken: String) : ApiFirebase(
            FirebaseMethod.APPLE, accessToken)
        class Phone(accessToken: String) : ApiFirebase(
            FirebaseMethod.PHONE, accessToken)
        class Anonimus(accessToken: String) : ApiFirebase(
            FirebaseMethod.ANONIMUS, accessToken)
    }

    companion object {
        fun fromCredentialResult(result: CredentialResult): ApiCredentials =
            when(result.method){
                is Firebase.Apple       -> ApiFirebase.Apple(
                    result.token
                )
                is Firebase.Google      -> ApiFirebase.Google(
                    result.token
                )
                is Firebase.Facebook    -> ApiFirebase.Facebook(
                    result.token
                )
                is PhoneAuthMethod -> {
                    if(result.token.isNullOrBlank())
                        Phone(
                            null,
                            (result.method as PhoneAuthMethod).phone,
                            (result.method as PhoneAuthMethod).code
                        )
                    else
                        ApiFirebase.Phone(
                            result.token
